...
>
> Three working modes are available for one, two, or three fits: **“Fit merge”** combines their contents for a joint purchase; **“Refit”** builds a set sufficient for any populated fit without duplicating shared items beyond the required quantity; **“Top-up”** treats fit 1 as existing equipment and fits 2 and 3 as targets, then lists the minimum shared package needed to move to either target.
>
...
- **release-1.1**
  ...
  - “Fit merge” and “Refit” now consider every populated fit.
  - In “Top-up,” fit 1 is the existing fit and fits 2 and 3 are alternative targets; the result is one missing-item package sufficient for either target.
  - Fits switch through internal tabs when the container is narrow, while all three fields appear side by side on wider screens.
  - Increased the maximum page width to make better use of large displays.
  - Automatic state names now include the complete headers of every populated fit.
...
- **One to three EFT fits.** Fits can be entered manually or pasted with a button. Extra blank lines, tabs, and leading or trailing whitespace are normalized automatically.
- **Three calculation modes.**
  - “Fit merge” adds quantities from every populated fit.
  - “Refit” uses the largest quantity for each matching item across all populated fits.
  - “Top-up” treats fit 1 as existing equipment and fits 2 and 3 as targets, returning the largest missing quantity required for either target.
...
- **Clear source indication.** Color markers identify items found in fit 1, 2, 3, or multiple fits.
...
- **Named states.** A state can be saved, opened, updated, or deleted. Its editable default name is built from the complete bracketed headers of every populated fit.
...
- **Responsive layout.** Three fit fields appear side by side on wide screens and switch through internal tabs when space is limited. The interface is designed for desktops, a 14-inch MacBook Pro, and mobile screens.
