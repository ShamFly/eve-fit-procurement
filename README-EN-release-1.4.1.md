# EVE Fit Procurement release-1.4.1
...
- **release-1.4.1**
  - Fixed the “Clear all fits” button overlapping the second fit's actions in the wide three-column layout.
  - Moved the global clear action into the shared fit-panel header, where it adds no extra vertical row and clearly applies to all three fields.
  - Made per-fit action buttons more compact while all three columns are visible.
- **release-1.4**
